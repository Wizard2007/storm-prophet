...
# df_array = []
# while year < year_end:
#     filename = 'all_data/dsc_fc_summed_spectra_' + str(year) + '_v01.csv'
#     df_tmp = pd.read_csv(filename)
#     num_columns = len(df_tmp.columns)
#     column_names = list(range(num_columns))
...
# X_data = pd.concat(df_array, axis=0, ignore_index=True, sort=False)
# # Reset the index if needed
# X_data.reset_index(drop=True, inplace=True)
#X_data = X_data.fillna(0)
X_data = pd.read_csv('discovr_summed_all.csv')

y_data = pd.read_csv('all_data/kyoto_minute_full.csv')
...
for i in range(len(X_data) - seq_length - prediction_horizon + 1):
    ...
    label_data = y_data[i:i + seq_length + prediction_horizon - 1]
...
model.save('lstm_all_andrew_24_24.h5')
plt.plot(history.history['loss'])
plt.grid(True)
plt.savefig('learning_all_andrew_24_24.png')
plt.show()

#model = load_model("lstm_all_andrew_24_24.h5")
...
with open("mean_dev_all_andrew_24_24.csv", "w", newline = "") as file:
